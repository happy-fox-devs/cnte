...
if (typeArgIndex === -1 || !args[typeArgIndex + 1]) {
  console.error('Error: Please specify release type using --type (patch|p, minor|m, major|M)');
  process.exit(1);
}
...
if (!releaseType) {
  console.error(`Error: Invalid release type '${releaseTypeRaw}'. Valid options are: patch, p, minor, m, major, M`);
  process.exit(1);
}
...
output.on('close', () => {
  ...
  try {
    execSync('git add package.json public/manifest.json', { stdio: 'inherit', cwd: rootDir });
    ...
    console.error('Error during git/gh operations. Please ensure git and gh CLI are configured correctly.', err.message);
    process.exit(1);
  }
});
